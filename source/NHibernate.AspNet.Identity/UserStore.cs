...
namespace NHibernate.AspNet.Identity
{
    ...
    public class UserStore<TUser> : UserStore<TUser, IdentityRole, ISession>
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        {
            get { return Context.Query<TUser>(); }
        }
        ...
        public async virtual Task AddToRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var roleEntity = await Task.FromResult(Roles.SingleOrDefault(r => r.Name.ToUpper() == roleName.ToUpper()));
            if (roleEntity == null)
            {
        ...
        public async virtual Task RemoveFromRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (roleEntity != null)
            {
                var userRole = await Task.FromResult(UserRoles.FirstOrDefault(r => roleEntity.Id.Equals(r.RoleId) && r.UserId.Equals(user.Id)));
                if (userRole != null)
                {
        ...
        public virtual async Task<bool> IsInRoleAsync(TUser user, string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var role = await Task.FromResult(Roles.SingleOrDefault(r => r.Name.ToUpper() == roleName.ToUpper()));
            if (role != null)
            {
                var userId = user.Id;
                var roleId = role.Id;
                return await Task.FromResult(UserRoles.Any(ur => ur.RoleId.Equals(roleId) && ur.UserId.Equals(userId)));
            }
            return false;
        }
        ...
        private IQueryable<TRole> Roles { get { return Context.Query<TRole>(); } }
        private IQueryable<IdentityUserClaim<TKey>> UserClaims { get { return Context.Query<IdentityUserClaim<TKey>>(); } }
        private IQueryable<IdentityUserRole<TKey>> UserRoles { get { return Context.Query<IdentityUserRole<TKey>>(); } }
        private IQueryable<IdentityUserLogin<TKey>> UserLogins { get { return Context.Query<IdentityUserLogin<TKey>>(); } }
        ...
        public async virtual Task<IList<Claim>> GetClaimsAsync(TUser user, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return await Task.FromResult(UserClaims.Where(uc => uc.UserId.Equals(user.Id)).Select(c => new Claim(c.ClaimType, c.ClaimValue)).ToList());
        }
        ...
        public async virtual Task ReplaceClaimAsync(TUser user, Claim claim, Claim newClaim, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var matchedClaims = await Task.FromResult(UserClaims.Where(uc => uc.UserId.Equals(user.Id) && uc.ClaimValue == claim.Value && uc.ClaimType == claim.Type).ToList());
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var userId = user.Id;
            var entry = await Task.FromResult(UserLogins.SingleOrDefault(l => l.UserId.Equals(userId) && l.LoginProvider == loginProvider && l.ProviderKey == providerKey));
            if (entry != null)
            {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (userLogin != null)
            {
                return await Task.FromResult(Users.FirstOrDefault(u => u.Id.Equals(userLogin.UserId)));
            }
            return null;
        }
        ...
        public async virtual Task<IList<TUser>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var role = await Task.FromResult(Roles.Where(x => x.Name.Equals(roleName)).FirstOrDefault());
